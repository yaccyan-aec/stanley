REM fts2 のルートパス
SET RootPath=c:\xampp\htdocs\fts_stanley
REM fts2 データのルートパス
SET DataPath=c:\data\fts_stanley
REM ========================================================
REM コマンドプロンプトからはこんなカンジで呼ぶ。
REM c:\xampp\htdocs\fts_v5\app\Console>chkmail
REM 
REM こんなふうに展開される↓
REM c:\xampp\htdocs\fts_v5\app>c:\xampp\htdocs\fts_v5\app\Console\cake Errmails.ChkMail -app c:\xampp\htdocs\fts_v5\app
REM ========================================================

SET DATE=%date:/=%
echo %DATE%
REM set time2=%time: =0%   
REM set TIME=%time2::=%
REM echo %TIME%
REM set NAME=%DATE%%TIME:~0,6%
SET NAME=%DATE:~7,1%
echo %NAME%


cd %RootPath%\app

REM c:\xampp\htdocs\fts_stanley\app\Console\cake CallOptions  >> %DataPath%/tmp/fts_option_%DATE%.log --app %RootPath%\app

c:\xampp\htdocs\fts_stanley\app\Console\cake CallOptions %1 %2 %3 --app %RootPath%\app

