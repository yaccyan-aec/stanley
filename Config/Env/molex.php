 ...
 *  2016.06.06
 */
// define('VALUE_Mail_TEST_TO','tsuyoshi.yoshiike@asahi-eg.co.jp');		//!< @brief 全てのメールを１アドレスで受信したいときに設定
 ...
 */
define('MY_DATA', '/data/stanley/data/molex');		// このアプリが使用するデータの基本パス
define('VALUE_Domain', 'dev-stanley-fts.asahi-eg.co.jp');		// shell だとドメインがとれないので
/**
 *  新規ファイル送信で使用する(仮)　2016/4/19　
 * constdef.phpでも設定しているがmolexの場合のみ再定義する。
 * 再定義したこちらの値が使用される。
 */
Configure::write('VALUE_Limit_Selection',array(3=>3));
...
//define('VALUE_Mail_Username','cat');			//!< @brief ユーザ （いらないかも）
//define('VALUE_Mail_Password','0cat');			//!< @brief パスワード （いらないかも）
...
define("PAGE_TITLE", "FTS5_molex");	//!< @brief Title ＜必須＞

define('HUMAN_TEST',false);					//!< @brief 画像認証の有無
...
Configure::write('Password_Security',
     array(
       ...
       'pwd' => array(
           ...
           'time_limit' => 45,    // 有効日数 0 : 無期限
           'history_limit' => 24  // 履歴保持 0 : なし
        ),
       'lockout' => array(
           'is_enable' => true,      // true : 有効 / false ：無効
...
define('MAIL_SEL_PREFIX','FTS_MolexDEV');		// 	本番
define('MAIL_SUBJECT_PREFIX','[FTS_MolexDEV]');		// メール表題の先頭につける決まり文句

define('VALUE_Copyright','Copyright (c) 2012- STANLEY ELECTRIC CO., LTD. All Rights Reserved.');
define('VALUE_Package','MolexDEV');		// 必須
...
define('VALUE_default_host' 		, '127.0.0.1');
...
// TO は複数宛先のときは,で羅列する
//--------------------------------------------
define('INQUIRY_TO','cat@jsy.ne.jp');
...
define('PRIVACY_POLICY','http://www.asahi-eg.co.jp/policy/');	// 問合せ用（必須）
