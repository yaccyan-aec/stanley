...
define('VALUE_default_database' 	, 'stanley');
 ...
 *  2016.06.06
 */
//define('VALUE_Mail_TEST_TO','cat@jsy.ne.jp');		//!< @brief 全てのメールを１アドレスで受信したいときに設定
 ...
 */
define('MY_DATA', '/data/vhost/stanley-fts02.cloud.niandc.ne.jp/stanley');		// このアプリが使用するデータの基本パス
define('VALUE_Domain', 'stanley-fts02.cloud.niandc.ne.jp');		// shell だとドメインがとれないので
//--------------------------------------------
// サーバ内フォルダ （最後の /　は不要）
//--------------------------------------------
...
define('DEBUG_MakingPermission',true);		//!< @brief [T] true: uri の経路チェックでダメならログイン画面に戻る
											//		（false はデバッグモード）
/**
...
define('VALUE_Mail_FromKey','Z_ISS_FTS01@stanley.co.jp');	//!< @brief 送信者メールアドレス
define('VALUE_Mail_FromVal','FTS Stanley');	//!< @brief 送信者名
// Config/email.php　の当該箇所をコメントアウトすること。
define('VALUE_Mail_Host','10.115.4.11');		//!< @brief メールサーバHost 新サーバ
define('VALUE_Mail_Port', 25);					//!< @brief メールサーバPort 25/587
define('VALUE_Mail_Timeout',60);				//!< @brief タイムアウト
define('VALUE_Mail_Username','');		//!< @brief ユーザ （いらないかも）
define('VALUE_Mail_Password','');			//!< @brief パスワード （いらないかも）
...
define('LOGO_LINK','http://www.stanley.co.jp/');		//!< @brief ロゴクリック時のリンク先
define("PAGE_TITLE", "StanleyFTS");	//!< @brief Title ＜必須＞

define('HUMAN_TEST',true);					//!< @brief 画像認証の有無
define('HUMAN_TEST_CODE_LENGTH',3);			//!< @brief  文字数
...
define('MAIL_SEL_PREFIX','FTS_Stanley');		// 	本番
define('MAIL_SUBJECT_PREFIX','[FTS_Stanley]');		// メール表題の先頭につける決まり文句

define('VALUE_Copyright','Copyright (c) 2012- STANLEY ELECTRIC CO., LTD. All Rights Reserved.');
define('VALUE_Package','FTS');						//!< @brief パッケージ名  ＜必須＞
 ...
 */
define('VALUE_content_limit_default' 	, 1);	//!< @brief デフォルトタイムリミット（日）
 ...
 */
define('VALUE_expapply_guide' 	, 'app/webroot/docs/default/jpn/guide/index.php');
//define('VALUE_expapply_format' 	, 'app/webroot/docs/default/jpn/fts2_man.pdf');
...
// TO は複数宛先のときは,で羅列する
//--------------------------------------------
define('INQUIRY_TO','yoshio_naruse@stanley.co.jp,kazumichi_tanabe@stanley.co.jp,tatsuro_ishiyama@stanley.co.jp,Z_ISS_FTS01@stanley.co.jp,fts-stanley@asahi-eg.co.jp');
//define('INQUIRY_TO','fts-stanley@asahi-eg.co.jp');
...
//--------------------------------------------------------------------
//このプラグインでのみ使用する設定があればこちらに記します。
//--------------------------------------------------------------------

if (!defined('ADDRESSBOOKS_PERSONAL_FLG')) {
  define('ADDRESSBOOKS_PERSONAL_FLG', false);  //個人版使用フラグ
}
if (!defined('ADDRESSBOOKS_SHARED_FLG')) {
  define('ADDRESSBOOKS_SHARED_FLG', true);    //共通版使用フラグ
}
/*********************************************
 *********************************************
...
//--------------------------------------------
// for PSCAN 2012.08.06
// 複数ドメイン併記　Stanley プラグインの bootstrap に移動しました。
//--------------------------------------------
define('VALUE_DomainForEmployee','http://stanley-fts02');	// 社内用
define('VALUE_DomainForVisitor','https://stanley-fts02.cloud.niandc.ne.jp');		// 社外用
