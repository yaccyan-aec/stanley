<?php
Configure::write('Config.theme', 'Nifco');
//Configure::write('Config.theme', 'default');
define('VALUE_System_Default_Lang','eng');		//!< @brief システムデフォルト言語 

define('VALUE_default_database' 	, 'fts_nidds');
 ...
 */ 
define('MY_DATA', 'C:'.DS.'data'.DS.'fts2');		// このアプリが使用するデータの基本パス
define('VALUE_Domain', '192.168.10.112');		// shell だとドメインがとれないので
//--------------------------------------------
// サーバ内フォルダ （最後の /　は不要）
//--------------------------------------------
...
define('VALUE_Mail_FromKey','shared-fts@jsy.co.jp');	//!< @brief 送信者メールアドレス 
define('VALUE_Mail_FromVal','shared-fts');	//!< @brief 送信者名 
...
define('LOGO_LINK','http://www.nifco.co.jp/');	//!< @brief ロゴクリック時のリンク先 
//define('LOGO_LINK','http://www.asahi-eg.co.jp/product/web-system/fts2/');	//!< @brief ロゴクリック時のリンク先 
define("PAGE_TITLE", "Nidds3");	//!< @brief Title ＜必須＞
...
//           'lockout_date' => '15 minutes' , // ロックアウト期間（日数）0 : 無期限
 ...
 //          'uri' => 'lockout' ,      // ロックアウト時に移動するページ（廃止）どこにも使っていない
           ...
           'is_chgpwd_demand' => false,   // ロックアウト解除時にパスワード変更を強制するかどうか
			// 以下追加項目
           'is_lockout_session_limit' => false, // セッションリトライ回数がオーバーしたらユーザをロックアウト（2012.10)
		   'is_human_test_retry_count' => true, // 画像認証エラーもカウントする（2012.10)
...
// Copyright
//--------------------------------------------
define('MAIL_SEL_PREFIX','Nidds3');		// 	本番
define('MAIL_SUBJECT_PREFIX','[Nidds3] ');		// メール表題の先頭につける決まり文句

define('VALUE_Copyright','Copyright (c) 2011- Nifco Inc. All rights reserved.');
define('VALUE_Package','Nidds3');						//!< @brief パッケージ名  ＜必須＞
...
// TO は複数宛先のときは,で羅列する
//--------------------------------------------
define('INQUIRY_TO','ohishi@asahi-eg.co.jp');
