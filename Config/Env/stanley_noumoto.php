...
// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓
define('VALUE_default_database' 	, 'stanley_noumoto');
 ...
 *  2016.06.06
 */
define('VALUE_Mail_TEST_TO','noumoto@jsy.co.jp');		//!< @brief 全てのメールを１アドレスで受信したいときに設定
 ...
 */
 // ↓ データ保管域は別個とする
define('MY_DATA', '/data/vhost/dev-stanley-fts.jsy.ne.jp/stanley_noumoto');		// このアプリが使用するデータの基本パス
// ↓
define('VALUE_Domain', 'dev-stanley-fts.jsy.ne.jp');		// shell だとドメインがとれないので
//--------------------------------------------
// サーバ内フォルダ （最後の /　は不要）
//--------------------------------------------
 ...
 */
 ...
 define('VALUE_Mail_FromKey','noumoto@jsy.co.jp');	//!< @brief 送信者メールアドレス
 define('VALUE_Mail_FromVal','FTS_NOUMOTO');	//!< @brief 送信者名
 // このサーバについては、port 25 でも 587 でも　username,password は不要（あるとエラーになる）
 // Config/email.php　の当該箇所をコメントアウトすること。
 define('VALUE_Mail_Host','second.jsy01.com');		//!< @brief メールサーバHost 新サーバ
 define('VALUE_Mail_Port', 587);					//!< @brief メールサーバPort 25/587
 define('VALUE_Mail_Timeout',60);				//!< @brief タイムアウト
 define('VALUE_Mail_Username','noumoto@jsy.co.jp');			//!< @brief ユーザ （いらないかも）
 define('VALUE_Mail_Password','0noumoto');			//!< @brief パスワード （いらないかも）
...
define('VALUE_file_size_limit' , 1*1024);	// ファイル最大サイズ (総量制限にも使用)
...
// ↓↓↓↓↓↓↓↓↓↓↓↓↓↓
define('MAIL_SUBJECT_PREFIX','[FTS_Stanley_noumoto]');		// メール表題の先頭につける決まり文句
...
// TO は複数宛先のときは,で羅列する
//--------------------------------------------
// define('INQUIRY_TO','stanley-admin@jsy.ne.jp');
define('INQUIRY_TO','noumoto@jsy.co.jp');
...
define('VALUE_DomainForEmployee','http://dev-stanley-fts.jsy.ne.jp');		// 社内用
define('VALUE_DomainForVisitor','https://dev-stanley-fts.jsy.ne.jp');		// 社外用
//--------------------------------------------
// ファイルシステムマウント (TFG)
//--------------------------------------------
/*本番
...
define('WIN_SERVER_IP','192.168.0.172');    // TFG サーバのIPアドレス
