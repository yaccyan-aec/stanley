 ...
 */ 
define('MY_DATA', 'C:'.DS.'data'.DS.'fts_shared');		// このアプリが使用するデータの基本パス
...
define('VALUE_Mail_FromKey','ohishi@jsy.co.jp');	//!< @brief 送信者メールアドレス 
define('VALUE_Mail_FromVal','ohishi');	//!< @brief 送信者名 
// このサーバについては、port 25 でも 587 でも　username,password は不要（あるとエラーになる）
// Config/email.php　の当該箇所をコメントアウトすること。
//define('VALUE_Mail_Host','mail.aec-fts.net');		//!< @brief メールサーバHost 新サーバ
define('VALUE_Mail_Port', 587);					//!< @brief メールサーバPort 25/587
define('VALUE_Mail_Timeout',30);				//!< @brief タイムアウト 
//define('VALUE_Mail_Username','web');			//!< @brief ユーザ （いらないかも）
//define('VALUE_Mail_Password','aec3750');			//!< @brief パスワード （いらないかも）
...
define('VALUE_Mail_Username','ohishi@jsy.co.jp');			//!< @brief ユーザ （いらないかも）
define('VALUE_Mail_Password','0ohishi');			//!< @brief パスワード （いらないかも）
...
define("PAGE_TITLE", "FTS5");	//!< @brief Title ＜必須＞
...
// Copyright
//--------------------------------------------
define('MAIL_SEL_PREFIX','FTS5');		// 	本番
define('MAIL_SUBJECT_PREFIX','[FTS5]');		// メール表題の先頭につける決まり文句

define('VALUE_Copyright','Copyright (c) 2014- ASAHI ENGINEERING CO .,LTD. All Rights Reserved.');
define('VALUE_Package','FTS5');						//!< @brief パッケージ名  ＜必須＞
